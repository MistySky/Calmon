...
- 关键构建设置：
  ...
  - `PRODUCT_BUNDLE_IDENTIFIER = com.calmon.CalMon`，`MARKETING_VERSION = 1.0`，`CURRENT_PROJECT_VERSION = 1`
  ...
  - 签名：`CODE_SIGN_STYLE = Manual`，`CODE_SIGN_IDENTITY = "-"`（ad-hoc），entitlement 只声明 `com.apple.security.app-sandbox = false`。
...
- `build/Build/Products/Release/CalMon.app/Contents/Info.plist`：`LSUIElement = true`、`CFBundleIdentifier = com.calmon.CalMon`、`CFBundleIconName = AppIcon`、`CFBundleShortVersionString = 1.0`、`LSMinimumSystemVersion = 27.0`。
- `Contents/Resources/` 仅含：`AppIcon.icns`、`Assets.car`、`CN-2024/2025/2026.json`、`2025/2026/2027.json`、`PrivacyInfo.xcprivacy`。没有 `docs/`、原型图或验收日志。

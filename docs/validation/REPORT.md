...
未执行（需人工/签名/环境）：① 实机首次授权→允许→自动识别中国节日日历；② 拒绝/撤销后状态与恢复；③ 覆盖安装后的系统授权继承（自签名稳定身份已实现，双版本实测仍待人工）；④ Mole/MacCalendar 同机并排比较；⑤ 深色/减少透明度/增加对比度/小屏；⑥ 真实睡眠唤醒、时区跳变。
...
**未执行（需人工/签名/环境）**：实机首次授权→允许→自动识别并读取；拒绝/撤销后状态与恢复；来源新增/删除后的实机发现；覆盖安装授权继承（自签名稳定身份已实现，仍待双版本实机实测）；深色/减少透明度/增加对比度/小屏；真实睡眠唤醒/时区跳变；Mole/MacCalendar 同机并排比较。DMG 未打。
